...
function ThemeSwitch({ dark, onToggle, mobile = false }: { dark: boolean; onToggle: () => void; mobile?: boolean }) {
  return <button type="button" onClick={onToggle} role="switch" aria-checked={dark} aria-label={dark ? "Switch to light mode" : "Switch to dark mode"} className={`flex items-center gap-3 rounded-xl border text-left transition-colors dark:border-slate-700 ${mobile ? "p-2" : "w-full px-3 py-2.5"}`}>
    ...
    {!mobile && <span className="text-xs font-medium text-slate-600 dark:text-slate-300">{dark ? "Dark mode" : "Light mode"}</span>}
  </button>;
}
...
export default function Page({ title, children }: { title: string; children?: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900 transition-colors dark:bg-slate-950 dark:text-slate-100">
    <aside className="fixed inset-y-0 left-0 hidden w-64 border-r bg-white p-6 dark:border-slate-800 dark:bg-slate-900 md:block">
      ...
      <div className="mt-5 border-t pt-5 dark:border-slate-800"><ThemeSwitch dark={dark} onToggle={toggleTheme} /></div>
      <button onClick={() => { localStorage.removeItem("pes_access_token"); window.location.href = "/login"; }} className="absolute bottom-6 left-6 right-6 rounded-lg bg-slate-50 p-3 text-left text-xs text-slate-500 transition-colors hover:bg-slate-100 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700">Sign out<br /><strong className="text-slate-700 dark:text-slate-200">Current account</strong></button>
    </aside>
